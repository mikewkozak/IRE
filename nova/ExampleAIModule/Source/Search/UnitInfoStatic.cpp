...
UnitInfoStatic::UnitInfoStatic()
{
	...
	for (int i = 0; i < MAX_UNIT_TYPE; ++i) {
		...
		for (int j = 0; j < MAX_UNIT_TYPE; ++j) {
			...
			if (!enemyType.isFlyer() && unitType.groundWeapon().damageAmount() > 0) {
				DPFval = static_cast<double>(unitType.groundWeapon().damageAmount() * unitType.groundWeapon().damageFactor() * unitType.maxGroundHits());
				...
				if (i == BWAPI::UnitTypes::Terran_Bunker)		 DPFval = (DPFval * 4.0) / unitType.groundWeapon().damageCooldown(); // consider Bunkers like 4 Marines
				...
				else											 DPFval = (DPFval / unitType.groundWeapon().damageCooldown());

			} else if (enemyType.isFlyer() && unitType.airWeapon().damageAmount() > 0) {
				DPFval = static_cast<double>(unitType.airWeapon().damageAmount() * unitType.airWeapon().damageFactor() * unitType.maxAirHits());
				...
				if (i == BWAPI::UnitTypes::Terran_Bunker)		 DPFval = (DPFval * 4.0) / unitType.airWeapon().damageCooldown(); // consider Bunkers like 4 Marines
				else if (i == BWAPI::UnitTypes::Protoss_Carrier) DPFval = (DPFval * 8.0) / 30.0; // can launch up to 8 interceptors each 30 frames
				else											 DPFval = DPFval / unitType.airWeapon().damageCooldown();
			}
			typeDPF[i][j] = DPFval;
		}
	}
	...
	for (int i = 0; i < MAX_UNIT_TYPE; ++i) {
		...
		if (unitType.groundWeapon().damageAmount() > 0) {
			double damage = unitType.groundWeapon().damageAmount() * unitType.groundWeapon().damageFactor() * unitType.maxGroundHits();
		...
		if (unitType.airWeapon().damageAmount() > 0) {
			double damage = unitType.airWeapon().damageAmount() * unitType.airWeapon().damageFactor() * unitType.maxAirHits();
